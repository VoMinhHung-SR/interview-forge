...
function TabSlider({
  ...
  return (
    <div className="relative flex rounded-lg bg-slate-100 p-1">
      <span
        className="absolute inset-y-1 w-[calc(50%-4px)] rounded-md bg-white shadow-sm transition-transform duration-200 ease-out"
        style={{
          left: "4px",
      ...
      {recent.slice(0, MAX_RECENT_DISPLAY).map((item) => (
        <li key={item.problemId}>
          <button
            ...
            className="flex w-full items-start justify-between gap-2 rounded-lg border border-slate-100 px-3 py-2.5 text-left transition hover:border-slate-200 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
          >
